terraform {
  #   Below commented out as using localstack and local state for purposes of this exercise
  #   Backend config would be provided per env in config/{env}/backend.conf
  #   backend "s3" {}
  required_providers {
    aws = {
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      created-by  = "platform_lab/s3" # Repo path - comes in handy for tracking down resource creation in large environments!
    }
  }
  # fudge localstack endpoints for testing
  endpoints {
